#!/usr/local/anaconda3/envs/mypython3/bin/python3
""" Usage:
    <file-name> --ds=DATASET_FILE --bi=IN_FILE --align=ALIGN_FILE --out=OUT_FILE --lang=LANG  [--debug]

    e.g.
    python load_alignments.py --ds=../data/aggregates/en.txt  --bi=../translations/google/en-es.txt --align=forward.en-es.align --lang=es --out=../data/human/google/es/es.pred.csv
"""
# External imports
import logging
import pdb
...
from evaluate import evaluate_bias, evaluate_bias_complete
...
LANGUAGE_PREDICTOR = {
    ...
    "de": lambda: GenderedArticlePredictor("de_core_news_md", get_german_determiners, GERMAN_EXCEPTION),
...
def align_bitext_to_ds(bitext, ds):
    ...
    """
    bitext_dict = dict([(src, (ind, tgt))
                       for ind, (src, tgt) in enumerate(bitext)])
...
if __name__ == "__main__":
    ...
    translated_profs, tgt_inds = get_translated_professions(
        align_fn, ds, bitext)
    ...
    logs_complete = evaluate_bias_complete(ds, gender_predictions)
